...
dossier_data = "C:/Users/Data 1/Desktop/citibike"
...
```
### Transformation des données création et suppression de certaines variables
```{r}
dd<-full[,40]
dd$substances<-as.character(dd$substances)
train_split<-cSplit_e(dd, "substances",sep = ", ", type = "character", drop = T, fill = 0)
names(train_split) <- paste("substances", 1:length(train_split), sep = "")
full<-cbind(full,train_split)

full$substances<-NULL
full$cv_id<-NULL 
full$libelle<-NULL
full<-as.h2o(full)
```
...
### ensemble learning
```{r}
train$logprix<-log(train$prix)
train$prix<-NULL
train$cv.id<-NULL
y <- "logprix"
#names(train[,47:1961]) <- NULL
x <- setdiff(names(train), y)
#x<-make.names(x, unique = TRUE, allow_ = FALSE)
train<-as.data.frame(train)

train<-as.h2o(train)

learner <- c("h2o.randomForest.wrapper", 
             "h2o.gbm.wrapper", "h2o.gbm.wrapper.3","h2o.deeplearning.wrapper", "h2o.deeplearning.wrapper.2")
...
validation<-train[1:500,]

pred <- predict(fit, test)
predictions <- as.data.frame(exp(pred$pred))[,1]
vrai <- as.data.frame(validation$logprix)
val<-cbind(predictions,vrai)
mape_error = function(y, ypred){mean(abs((y - ypred)/y))*100}
 mape        = mape_error(y = exp(val$logprix), ypred = exp(val$predict)); print(mape)
perf <- h2o.ensemble_performance(fit, newdata = validation)
...
```{r, message= F, warning= F}
# on entraine de nouveau le modele, cette fois sur l'intÃ©gralite des donnees
